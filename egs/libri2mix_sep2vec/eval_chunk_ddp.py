...
def main(rank, world_size, conf):
    ...
    for idx, data in enumerate(tqdm(test_loader)):
        ...
        # est_source = model(mix, enroll, delay_src)
        # est_source = model.module.forward_streaming_debug(mix, enroll, kernel_size=16, step_size=8, time_delay=conf["train_conf"]["data"]["delay_sample"]).unsqueeze(1)
        est_source = model.module.forward_streaming(mix, enroll, delay_src, kernel_size=16, step_size=8, time_delay=conf["train_conf"]["data"]["delay_sample"]).unsqueeze(1)
