...
def main(conf):
    ...
    if not conf["from_checkpoint"]:
        ...
        #                            ) 
        # model = TimeDomainSpeakerBeamPredictHelp(i_adapt_layer=7, adapt_layer_type='mul', adapt_enroll_dim=128, causal=True, stride=16)
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        # est_source = model.forward_streaming_debug(mix.unsqueeze(0), enroll.unsqueeze(0), delay_src.unsqueeze(0)).unsqueeze(0)
        est_source = model.forward_streaming_debug(mix.unsqueeze(0), enroll.unsqueeze(0), kernel_size=16, step_size=8, time_delay=16).unsqueeze(0)
    ...
    all_metrics_df.to_csv(os.path.join(local_save_dir, "all_metrics.csv"))
    ...
    with open(os.path.join(local_save_dir, "final_metrics.json"), "w") as f:
