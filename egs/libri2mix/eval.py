...
from models_direct.td_speakerbeam import TimeDomainSpeakerBeam
...
def main(conf):
    ...
    if not conf["from_checkpoint"]:
        ...
        model = TimeDomainSpeakerBeam(**conf["train_conf"]["filterbank"],
                                   ...
                                   **conf["train_conf"]["enroll"],
                                    causal=False, #  update for non-causal
                                    norm_type='bN', # update for non-causal
                                   ) 
    ...
    local_save_dir = os.path.join(conf["out_dir"], "out_noncausal_direct/")
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        # print("est src shape: ", est_source.shape)
